...
#[test]
    ...
    #[test]
    fn chat() {
        let msg_original = Message::ChatMessage(String::from("jean miche muche"), String::from("la mouche qui pète"), String::from("renaaaaaard ptit batard!"));
        let msg_serialized = msg_original.encode();
        let msg_deserialized = Message::decode(msg_serialized).unwrap();
        assert_eq!(msg_original, msg_deserialized);

        let msg_original = Message::ChatMessage(String::new(), String::new(), String::new());
        let msg_serialized = msg_original.encode();
        let msg_deserialized = Message::decode(msg_serialized).unwrap();
        assert_eq!(msg_original, msg_deserialized);
    }
